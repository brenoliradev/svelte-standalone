<script lang="ts">
	...
	const initScript = () => {
		if (browser) {
			if (window) {
				window.once_with_typescript.start(defaultConfig);
			}
		}
	};
	...
	const stopScript = () => {
		if (browser) {
			if (window) {
				window.once_with_typescript.stop();
			}
		}
	};

	onDestroy(() => {
		if (window.once_with_typescript) {
			window.once_with_typescript.stop();
		}
	})
</script>
...
<svelte:head>
	<script type="module" src="/dist/standalone/once_with_typescript.min.js"></script>
</svelte:head>
...
<main class="p-2">
	<button
		...
		on:click={initScript}>init once_with_typescript script</button
	>
	<button
		class="rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
		on:click={stopScript}>stop once_with_typescript script</button
	>
