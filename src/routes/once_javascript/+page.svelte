<script>
	...
	import { defaultConfig } from '@/_standalone/once_javascript/config.js';

	const initScript = () => {
		if (browser) {
			if (window) {
				window.once_javascript.start(defaultConfig);
			}
		}
	};
...
<svelte:head>
	<script type="module" src="/dist/standalone/once_javascript.min.js"></script>
</svelte:head>
...
<main class="p-2">
	<button
		...
		on:click={initScript}>init once_javascript script</button
	>
	<button
		class="rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
		on:click={stopScript}>stop once_javascript script</button
	>
