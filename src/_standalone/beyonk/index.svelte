<script>
	...
	function demo() {
		notifier[type === 'default' ? 'send' : type](text, {
			timeout,
			persist,
			showProgress,
...
<form on:submit|preventDefault={demo}>
	<label class="form" for="text"> Notification text </label>
	<input id="text" type="text" class="form" bind:value={text} />

	<label class="form" for="type"> Notification type </label>
	...
	<div>
		...
		<label for="persist" class="form"> Persist </label>
	</div>

	<div>
		<input type="checkbox" class="form" bind:checked={showProgress} id="showProgress" />
		<label for="showProgress" class="form"> Show Progress </label>
	</div>

	<div>
		<input type="checkbox" class="form" bind:checked={showIcon} id="showIcon" />
		<label for="showIcon" class="form"> Show Icon </label>
	</div>

	<button class="form"> notify </button>
</form>
